...
function toggleAuthenticationModeView() {
    ...
    if (isSignUpMode) {
        ...
        if (forgotArea) forgotArea.style.display = "none"; // Hide forgot password on registration screen
    } else {
        title.textContent = "Sign In to Workspace";
        submitBtn.textContent = "Sign In";
        switchText.textContent = "Don't have an account?";
        toggleLink.textContent = "Create Account";
        if (forgotArea) forgotArea.style.display = "block"; // Show forgot password back on login screen
    }
}
...
// 👁️ Password Visibility Toggler Engine for Mobile Viewports
...
function appendMessageUI(text, sender, msgId, shouldStream = false) {
    ...
    if (sender === 'user') {
        ...
        // Check if this is an error message block to display a retry action button layout option
        ...
        if (shouldStream) {
            ...
            function renderWordChunk() {
                if (index < words.length) {
                    // 📱 SMART MOBILE LOCK ENGINE: Save scroll position before layout changes
                    const isUserAtBottom = (box.scrollHeight - box.clientHeight - box.scrollTop) <= 120;
                    
                    combinedText += words[index] + " ";
                    textTarget.innerHTML = marked.parse(combinedText);
                    
                    // Only snap window tracking downwards if you haven't intentionally scrolled up
                    if (isUserAtBottom) {
...
async function processMessage() {
    ...
    // 🛑 SECURE MONETIZATION GATEWAY CHECK
    if (typeof TenoraBilling !== 'undefined' && !TenoraBilling.canUserSendMessage()) {
        alert("Access Denied: Workspace execution parameters fully depleted. Please refill credits or watch an ad.");
        const upgradeModal = document.getElementById('premium-upgrade-modal');
        if (upgradeModal) upgradeModal.style.display = 'flex';
        TenoraBilling.updateInterfaceVisibility();
        return;
    }
    ...
    // Core database handler processes deductions automatically on user roles
    ...
    if (navigator.onLine) {
        try {
            ...
            // 🧠 HISTORICAL LOG READ OPERATION FOR FULL GLOBAL MEMORY SCOPE
                ...
                const crossChatPromise = new Promise((resolve) => {
                    ...
                    request.onsuccess = (event) => {
                        ...
                        if (cursor) {
                            ...
                            // Map the message arrays into standard model payload tokens
                            ...
                            if (session.id === currentSessionId) {
                                ...
                                // Inject historical context headers to alert the model of separate chat threads
                            ...
                            // Combine global historical context with the active session workspace
           ...
           // viewport lock right before API handshake fires
            const box = document.getElementById('chat-container');
            ...
            const res = await fetch('https://stsnwvlihfwmexivtcjm.supabase.co/functions/v1/hyper-function', {
                ...
                body: JSON.stringify({ 
                    prompt: promptText,
                    history: structuralHistory // Transmits full cross-chat archival map to the edge backend
                })
            });

            // PROCESS MESSAGE API RESPONSE VALIDATION LAYER
            if (!res.ok) {
...
function writeToDB(sessionId, messageBlock) {
    return new Promise((resolve) => {
        ...
        getReq.onsuccess = async () => {
            // 🧠 SMART TITLING ENGINE: Truncate strings past 26 characters to keep sidebar clean
            ...
            tx.oncomplete = async () => {
                ...
                if (messageBlock.role === 'user') {
                    if (typeof TenoraBilling !== 'undefined') {
                        // Securely await the cloud transaction save operation
                        await TenoraBilling.deductUsageCredit();
                    }
                }
                renderHistorySidebar(); 
                resolve(); 
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    if (mainChatView && mobileSidebar) {
        // Automatically dismisses the panel if a user clicks or taps anywhere on the active main screen

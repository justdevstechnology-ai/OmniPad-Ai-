const CACHE_NAME = "omnipad-cache-v1";
...
self.addEventListener("fetch", (event) => {
  // 🛑 CRITICAL SECURITY SHIELD: Never intercept cloud backend API queries or Firebase traffic
  if (event.request.url.includes('/api/') || event.request.url.includes('firebase') || event.request.url.includes('supabase')) {
    return;
  }
